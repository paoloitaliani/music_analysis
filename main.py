...
def welcome(message):
    introduction_message = "Hey this is your music recommendation bot. I will help you find songs similar to the" \
                           " one that you will provide me. What is the name of the song that you have in mind?"
    sent_msg = bot.send_message(message.chat.id, introduction_message)

    bot.register_next_step_handler(sent_msg, track_handler)


def track_handler(pm):
    track = pm.text
    artist_message = "Good! What's the artist name?"
    sent_msg = bot.send_message(pm.chat.id, artist_message)
    bot.register_next_step_handler(sent_msg, recommendation_handler, track)


def recommendation_handler(pm, track):
    artist = pm.text
    ...
    bot.send_message(pm.chat.id, recommendations[0])
